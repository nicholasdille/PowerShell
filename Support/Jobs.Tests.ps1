...
Describe 'Invoke-Queue' {
    It 'Returns a single object for a single input object' {
        (Invoke-Queue -InputObject @(1) -Scriptblock {}) -isnot [array] | Should Be $true
    }
    ...
    #    Mock Invoke-MyCode {}
    #    Invoke-Queue -InputObject @(1, 2) -Scriptblock $sb | Wait-Job | Out-Null
    #    Assert-MockCalled Invoke-MyCode -Exactly -Times 2 -Scope It
    #}
    It 'Successfully completes jobs on successful code' {
        $j = Invoke-Queue -InputObject @(1, 2) -Scriptblock {} | Wait-Job
